...
const dropScreen = $('dren');
const vizScreen  = $('vien');
const dropZone   = $('drne');
const fileInput  = $('fiut');
const canvas     = $('c');
...
$('newn').addEventListener('click',   () => fileInput.click());
...
function setupAudio() {
  ...
  analyser.smoothingTimeConstant = parseFloat($('smth').value);
...
function bindSlider(id, outId, decimals) {
  const el = $(id), out = $(outId);
  el.addEventListener('input', () => { out.textContent = parseFloat(el.value).toFixed(decimals); });
}
bindSlider('bant','svrs',0);
bindSlider('gain','svin',1);
bindSlider('smth','svsh',2);
bindSlider('liw','svlw',1);

$('bant').addEventListener('input', () => { $('sbrs').textContent = $('bant').value; });
$('smth').addEventListener('input', () => { if (analyser) analyser.smoothingTimeConstant = parseFloat($('smth').value); });
$('moel').addEventListener('change', () => { $('smde').textContent = $('moel').value.toUpperCase(); waterfallBuf = []; rotAngle = 0; });

['glow','trail','peak','mirror','grid','pulse','rotate'].forEach((k,i) => {
  const ids = ['fxow','fxil','fxak','fxor','fxid','fxse','fxte'];
  const btn = $(ids[i]);
...
function render() {
  ...
  const mode  = $('moel').value;
  const gain  = parseFloat($('gain').value);
  const nbars = parseInt($('bant').value);
  const lw    = parseFloat($('liw').value);
  const bg    = bgMap[$('bgel').value] || '#06060a';
  ...
  $('sfeq').textContent = domHz < 1000 ? domHz+'Hz' : (domHz/1000).toFixed(1)+'k';
  $('svol').textContent  = Math.round(avgVol)+'%';
  $('smde').textContent = mode.toUpperCase();
 ...
 // idk why this works but it does
  ...
  const vBars = document.querySelectorAll('.vub');
  const v0 = avgVol;
  ...
  if(bassVol>0.6&&bassVol>lastBeatVol*1.3){
    ...
    if(bpmTimes.length>=4){
      ...
      if(detectedBPM>40&&detectedBPM<220){
        $('sbpm').textContent=detectedBPM;
        $('bpay').textContent=detectedBPM+' BPM';
      }
    }
    beatFlash=1;
    if(window.logBeat)logBeat(detectedBPM);
    $('bpay').classList.add('beat');
    setTimeout(()=>$('bpay').classList.remove('beat'),80);
    flashAlpha=0.18;
  }
  lastBeatVol=bassVol;
}
...
document.addEventListener('keydown',e=>{
  ...
  switch(e.code){
    ...
    case 'ArrowRight':{
      const idx=(modeList.indexOf($('moel').value)+1)%modeList.length;
      $('moel').value=modeList[idx];
      $('moel').dispatchEvent(new Event('change'));
      break;
    }
    case 'ArrowLeft':{
      const idx=(modeList.indexOf($('moel').value)-1+modeList.length)%modeList.length;
      $('moel').value=modeList[idx];
      $('moel').dispatchEvent(new Event('change'));
      break;
    }
    ...
    case 'KeyG':
      FX.glow=!FX.glow;
      $('fxow').classList.toggle('on',FX.glow);
      break;
    case 'KeyT':
      FX.trail=!FX.trail;
      $('fxil').classList.toggle('on',FX.trail);
      break;
    case 'KeyP':
      FX.pulse=!FX.pulse;
      $('fxse').classList.toggle('on',FX.pulse);
      break;
    case 'KeyK':
      FX.peak=!FX.peak;
      $('fxak').classList.toggle('on',FX.peak);
      break;
    case 'KeyX':
      FX.mirror=!FX.mirror;
      $('fxor').classList.toggle('on',FX.mirror);
      break;
    case 'KeyR':
      FX.rotate=!FX.rotate;
      $('fxte').classList.toggle('on',FX.rotate);
      break;
    case 'KeyS':
      snap();
      break;
...
function recStart(){
  if(!ac){ toast('play audio first'); return; }
  ...
  mediaRec.onstop=()=>{
    ...
    a.download=($('trme').textContent||'viz')+'.webm';
    a.click();
    $('rege').style.display='none';
    $('btec').textContent='Record';
    $('btec').classList.remove('on');
    $('reer').style.display='none';
  ...
  recTimerInterval=setInterval(()=>{
    ...
    $('reer').textContent=`${m}:${sec.toString().padStart(2,'0')}`;
  },500);
  $('rege').style.display='block';
  $('btec').textContent='Stop';
  $('btec').classList.add('on');
  $('reer').style.display='inline';
...
$('btec').addEventListener('click',()=>{
  mediaRec&&mediaRec.state==='recording'?stopRec():recStart();
});

function snap(){
  const a=document.createElement('a');
  a.href=canvas.toDataURL('image/png');
  a.download=($('trme').textContent||'viz')+'.png';
  a.click();
  toast('snapshot saved');
}
$('btap').addEventListener('click',snap);
